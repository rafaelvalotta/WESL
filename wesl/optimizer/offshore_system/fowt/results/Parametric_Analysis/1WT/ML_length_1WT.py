...
#%%INPUT DATA
ws = np.array([4] + list(range(6, 11, 2)) + [11] + list(range(13, 26, 2)))
wd = np.linspace(0, 360, 360, endpoint=False)        #30 wd
...
for ml_length_index in ml_length:
    ...
    FOWT_Spar_def = FOWT_Spar(tot_mass=8066048,hub_height=90,msl_to_bottom=120,msl_to_CB=62,
                                  ...
                                  msl_to_fairlead=70,mooring_line_length= ml_length_index ,radius_to_anchor=853.87,sea_depth=320,
    ...
    import copy
    FOWT_Spar_array = [copy.deepcopy(FOWT_Spar_def) for _ in range(16)]
    
    # FOWT_Spar_array[1].mooring_line_length = 962.2
    # FOWT_Spar_array[5].mooring_line_length = 962.2
    # FOWT_Spar_array[9].mooring_line_length = 962.2
    # FOWT_Spar_array[13].mooring_line_length = 962.2
    
    # FOWT_Spar_array[2].mooring_line_length = 932.2
    # FOWT_Spar_array[6].mooring_line_length = 932.2
    # FOWT_Spar_array[10].mooring_line_length = 932.2
    # FOWT_Spar_array[14].mooring_line_length = 932.2
    
    # FOWT_Spar_array[3].mooring_line_length = 902.2
    # FOWT_Spar_array[7].mooring_line_length = 902.2
    # FOWT_Spar_array[11].mooring_line_length = 902.2
    # FOWT_Spar_array[15].mooring_line_length = 902.2    
    ...
    #MY SITE
    fowt_x_base = np.array([0],dtype='float64')
    fowt_y_base = np.array([0],dtype='float64')
    ...
    for ws_index in range(len(ws)):
        for wd_index in range(len(wd)):
            ...
            for wt_index in index:
                ...
                thrust_eff_new = FOWT_Spar_array[wt_index].get_thrust(WS_eff_WT_new,tilt_in_deg=None)
                
                # Get the rotor centre displacemenent for the initial iteration
                xr_new, yr_new, zr_new, gamma_new = FOWT_Spar_array[wt_index].solve_static_movement_direct(thrust_eff_new, wd[wd_index],MS_config)
                ...
                while True:
                    ...
                    thrust_eff_new = FOWT_Spar_array[wt_index].get_thrust(WS_eff_WT_new,tilt_in_deg=tilt_temp[wt_index])
    ...
    save_results(results, f'./results/Parametric_Analysis/1WT/ML_length/{ml_length_index}.pkl')
