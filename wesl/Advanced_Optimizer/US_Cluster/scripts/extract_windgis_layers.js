 ...
 *    Inter-Array Cables, Export Cables, Mooring Lines).
 * 4. Pan/zoom (or use the project search) to the area/cluster you
 *    want to capture, and wait for the map to stay still (tiles fully loaded).
 * 5. Open the DevTools Console (F12) and paste this entire file. It will:
 *      - find the mapboxgl.Map instance inside the React tree
 *      - list available vector sources/layers
 *      - extract currently loaded features from each layer
 *      - place everything in `window.__extracted` as GeoJSON FeatureCollections
 * 6. Copy `JSON.stringify(window.__extracted)` and save as .json, or download
 *    directly with `downloadExtracted()` (defined at the end of this script).
 *
 ...
 * fragments that share the same key property - use the companion Python script
 * `dissolve_tiles.py` (shapely.ops.unary_union) to reconstruct the single,
 * continuous polygon for each lease area. Points (turbines, substations) only
 * duplicate if they fall exactly on a tile edge - deduping by rounded coordinates solves it.
 */
...
(function () {
  ...
  console.log(
    'Example tile URL template:',
    style.sources[vectorLayers[0]?.source]?.tiles?.[0]
  );
  ...
  for (const l of vectorLayers) {
    ...
    const key = l.id; // use layer id as key (unique and readable)
    extracted[key] = {
  ...
  console.log('Run downloadExtracted() to download everything as a .json.');
  ...
  window.downloadExtracted = function downloadExtracted() {
    ...
    a.href = url;
    a.download = 'windgis_extracted_raw.json';
    a.click();
