...
print(f"Cluster existente (13 leases, infill estocastico): {len(cluster_all_x)} turbinas neste exemplo")"""
))
...
print(f"{len(_coast_xy)} pontos de costa (batimetria 0-5m) usados como proxy de conexao")"""
))
...
print(f"Grid: {ELIGIBILITY_GRID['nx']}x{ELIGIBILITY_GRID['ny']} = "
      f"{ELIGIBILITY_GRID['nx']*ELIGIBILITY_GRID['ny']:,} celulas, "
      f"construida em {ELIGIBILITY_GRID['build_time']:.1f}s (uma vez so, reaproveitada em todo o MC)")"""
))
...
print(f"Area livre: {_free.sum()*_cell_km2:.0f} km2 em {_n_components} fragmentos")
print(f"Maior fragmento: {_sizes_km2[0]:.0f} km2 ({_sizes_km2[0]/_free.sum()/_cell_km2*100:.1f}% da area livre total)")
print("Conclusao: nao esta fragmentado -- o gargalo e o encaixe geometrico dentro da faixa sorteada, nao a area total.")"""
))
...
for seed in range(N_SEEDS):
    ...
    print(f"seed {seed}: {len(scenario)} fazendas novas colocadas, cluster fill = {len(cx)} turbinas")
elapsed = time.time() - t0

log_df = pd.DataFrame(all_logs)
req, deliv = log_df["target_mw"].sum(), log_df["delivered_mw"].sum()
print(f"\\n{N_SEEDS} sorteios em {elapsed:.0f}s ({elapsed/N_SEEDS:.1f}s/sorteio)")
print(f"Requisitado: {req:.0f} MW | Entregue: {deliv:.0f} MW ({deliv/req*100:.1f}%)")

log_df.to_csv(RESULTS_DIR / "scenario_generation_log.csv", index=False)
print(f"Log salvo em {RESULTS_DIR / 'scenario_generation_log.csv'}")"""
))
